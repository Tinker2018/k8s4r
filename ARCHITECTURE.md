...
2. **Job** - 任务定义，包含 TaskGroup 模板和 RobotSelector
3. **TaskGroup** - 任务组，包含 count 字段用于副本数控制
4. **Task** - 任务执行实例（类比 Kubernetes Pod）

**资源层级关系**（参考 Kubernetes Deployment → ReplicaSet → Pod）：
```
Job (用户创建)
  └─> TaskGroup (Job Controller 创建)
        └─> Task (TaskGroup Controller 创建)
```
...
│  │  │  - 推送Task到gRPC Stream                              │       │ │
│  │  │  - 监控Task状态变化                                    │       │ │
│  │  │  - 不创建资源（Task由TaskGroup Controller创建）       │       │ │
│  │  └──────────────────────────────────────────────────────┘       │ │
│  │                                                                   │ │
│  │  ┌──────────────────────────────────────────────────────────┐   │ │
...
```
   ┌─────────┐
   │ Pending │  ← TaskGroup Controller 创建 Task
   └────┬────┘
        │ TaskGroup Controller 完成调度
        │ 设置 spec.targetRobot
        ▼
   ┌──────────┐
   │Scheduled │  ← Task Controller 推送到 gRPC Stream
   └────┬─────┘
        │ Server 转发到 MQTT
        │ Agent 接收并开始执行
        ▼
...
```
   ┌─────────┐
   │ Pending │  ← Job Controller 创建 TaskGroup
   └────┬────┘
        │ TaskGroup Controller 选择 robots
        │ 填充 Status.AssignedRobots
        ▼
   ┌──────────┐
   │Scheduled │  ← TaskGroup Controller 创建 Tasks
   └────┬─────┘
        │ 至少一个 Task 开始运行
        ▼
   ┌────────┐
...
```
   ...
   │ Pending │  ← 用户创建 Job
   └────┬────┘
        │ Job Controller 创建 TaskGroups
        │ 至少一个 TaskGroup 开始运行
        ▼
   ┌────────┐
   │ Running│  ← 聚合 TaskGroup 状态
   └────┬───┘
        │ 所有 TaskGroup 完成
        ├──────────────┬──────────────┐
        │              │              │
...
### Controller 职责划分（遵循 Kubernetes 设计模式）

**设计原则**：每个 Controller 只负责创建和管理下一层级的资源，类似 Deployment → ReplicaSet → Pod

| Controller | 创建资源 | 调度决策 | 状态聚合 | 备注 |
|-----------|---------|---------|---------|------|
| **Job Controller** | TaskGroup | ❌ | TaskGroup → Job | 类似 Deployment Controller |
| **TaskGroup Controller** | Task | ✅ 选择 robots | Task → TaskGroup | 类似 ReplicaSet Controller<br>根据 count 创建多个 Task |
| **Task Controller** | ❌ | ❌ | ❌ | 类似 kubelet<br>只推送到 gRPC Stream 和监控状态 |
| **Robot Controller** | ❌ | ❌ | ❌ | 只处理注册和心跳 |

**关键流程**：
1. **Job Controller**：检测 Job 创建 → 为每个 taskGroups 创建 TaskGroup CRD
2. **TaskGroup Controller**：
   - 调度阶段：选择 robots，填充 `Status.AssignedRobots`，状态设为 `Scheduled`
   - 创建阶段：根据 `AssignedRobots` 创建 Task 实例（每个 replica 一个）
   - 聚合阶段：收集 Task 状态，更新 TaskGroup 状态
3. **Task Controller**：
   - 检测 Task 创建 → 推送到 gRPC Stream → MQTT → Agent
   - 监控 Task 状态变化（Pending → Scheduled → Running → Completed/Failed）
   - **不创建任何资源**
