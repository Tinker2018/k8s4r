...
public:
     ...
     * @locale zh
     * @type api
     * @brief 退出并销毁调用 createGameRoom{@link #IRTCEngine#createGameRoom} 所创建的房间实例。
     * @list 房间管理
     */
    /**
     * @locale en
     * @type api
     * @brief  Leave and destroy the room instance created by calling createGameRoom{@link #IRTCEngine#createGameRoom}.
     ...
     * @type api
     * @brief 加入房间。 <br>
     *        调用 createGameRoom{@link #IRTCEngine#createGameRoom} 创建房间后，调用此方法加入房间，同房间内其他用户进行音频通话。
     ...
     * @return
     *        - 0: 调用成功。如果用户是房间内可见用户，远端用户收到 onUserLeave{@link #IRTCRoomEventHandler#onUserLeave} 回调通知。
     *        - < 0: 调用失败，参看 ReturnStatus{@link #ReturnStatus} 获得更多错误说明。
     * @note
     *       - 加入房间后，必须调用此方法结束通话，否则无法开始下一次通话。
     *       - 此方法是异步操作，调用返回时并没有真正退出房间。真正退出房间后，本地会收到 onRoomStateChanged{@link #IRTCRoomEventHandler#onRoomStateChanged} 回调通知。你必须在收到 onRoomStateChanged{@link #IRTCRoomEventHandler#onRoomStateChanged} 回调后，再销毁房间或引擎，或调用 joinRoom{@link #IGameRoom#joinRoom} 再次加入房间。
     * @list 房间管理
     */
    /**
     * @locale en
     ...
     * @note
     *        - When visible users leave the room, others in the room will receive onUserLeave{@link #IRTCRoomEventHandler#onUserLeave}.
     *        It is an asynchronous operation, and the call returns without actually exiting the room. When actually exiting the room, you will receive onRoomStateChanged{@link #IRTCRoomEventHandler#onRoomStateChanged}. If the engine is destroyed immediately after this method is called, you will not receive onRoomStateChanged{@link #IRTCRoomEventHandler#onRoomStateChanged}.
     */
     ...
     *        -2: 传入的room_id为空导致失败
     * @notes 不可与 EnableAudioSend{@link #IGameRTCRoom#EnableAudioSend} 同时调用。
     */
    /**
     * @locale en
     * @type api
     ...
     * @notes Cannot be called simultaneously with EnableAudioSend{@link #IGameRTCRoom#EnableAudioSend}.
     */
     ...
     *        0:  表示参数检查通过，不代表打开麦克风会成功，比如房间不存在
     *        -2: 传入的room_id为空导致失败
     * @notes 不可与 EnableMicrophone{@link #IGameRTCRoom#EnableMicrophone} 同时调用。
     */
    /**
     * @locale en
     * @type api
     ...
     *        0:  It means that the parameter check has passed, but it does not mean that opening the microphone will succeed. For example, the room does not exist.
     *        -2: The room_id passed in is empty, causing failure.
     * @notes Cannot be called at the same time as EnableMicrophone{@link #IGameRTCRoom#EnableMicrophone}.
     */
     ...
     * @param user_id
     *        User ID, a non-empty string with a maximum length of 128 bytes. The supported character set range is:
