...
public:
     ...
     * @locale zh
     * @type callback
     * @brief 该接口将于 3.64 版本废弃，请使用onUserMessageReceivedOutsideRoom{@link #IRTCEngineEventHandler#onUserMessageReceivedOutsideRoom-2} 代替。
     ...
     * @note 当调用 sendUserMessageOutsideRoom{@link #IRTCEngine#sendUserMessageOutsideRoom} 或 sendUserBinaryMessageOutsideRoom{@link #IRTCEngine#sendUserBinaryMessageOutsideRoom} 发送消息后，会收到此回调。
     * @list 消息
     */
    /**
     * @locale en
     ...
     * @note Receive this callback when a message is sent by calling sendUserMessageOutsideRoom{@link #IRTCEngine#sendUserMessageOutsideRoom} or sendUserBinaryMessageOutsideRoom{@link #IRTCEngine#sendUserBinaryMessageOutsideRoom}.
     * @list Messaging
     */
     ...
     * @param size Binary message length
     * @list Messaging
     */
     ...
     * @param stream_id 收到 SEI 消息的视频流id
    ...
    */
     ...
     * @param stream_id 开启音频采集的远端用户发布的流id
     ...
     * @param stream_id 关闭音频采集的远端用户发布的流id
     ...
     *        4. 在 1 条件下，订阅端取消订阅视频后，再次订阅视频（调用接口包括 subscribeAllStreamsVideo{@link #IRTCRoom-subscribeallstreamsVideo}，pauseAllSubscribedStreamVideo{@link #IRTCRoom-pauseallsubscribedstreamsVideo}/resumeAllSubscribedStreamVideo{@link #IRTCRoom-resumeAllSubscribedStreamVideo}）
     * @param stream_id 远端视频流 ID。
     * @param stream_info 远端视频流信息。参看 StreamInfo{@link #StreamInfo}。
     * @param info 视频帧信息。参看 VideoFrameInfo{@link #VideoFrameInfo}。
     ...
     * @param index Audio source. See IAudioSource{@link #IAudioSource}.
     ...
     * @hidden(Linux,iOS)
     * @type callback
     * @brief 外部采集时，调用 setOriginalScreenVideoInfo{@link #IRTCEngine#setOriginalScreenVideoInfo} 设置屏幕或窗口大小改变前的分辨率后，若屏幕采集模式为智能模式，你将收到此回调，根据 RTC 智能决策合适的帧率和分辨率积（宽*高）重新采集。
     ...
     * @hidden(Linux,iOS)
     * @type callback
     * @brief After calling setOriginalScreenVideoInfo{@link #IRTCEngine#setOriginalScreenVideoInfo} to set the original width and height of the external shared-screen stream and setting the encoding mode to the automatic mode, you will receive this callback to re-capture the stream based on the recommended pixel and framerate by RTC.
    ...
    */
     ...
     * @locale zh
     * @type callback
     * @brief 调用 takeSnapshotToFile{@link #IRTCVideo#takeSnapshotToFile} 截取视频画面时，会收到此回调报告截图是否成功，以及截取的图片信息。
     ...
     * @param task_id 截图任务的编号，和 takeSnapshotToFile{@link #IRTCVideo#takeSnapshotToFile} 的返回值一致。
     * @list 高级功能
     */
    /**
     * @locale en
     * @type callback
     * @brief Triggered after calling takeSnapshotToFile{@link #IRTCVideo#takeSnapshotToFile} to report whether the snapshot is taken successfully and provide details of the snapshot image.
     ...
     * @param task_id The index of the snapshot task, which matches the return value of takeSnapshotToFile{@link #IRTCVideo#takeSnapshotToFile}.
     * @list Advanced Features
     */
     ...
     * @type callback
     * @brief 调用 takeSnapshotToFile{@link #IRTCVideo#takeSnapshotToFile} 截取视频画面时，会收到此回调报告截图是否成功，以及截取的图片信息。
     * @param stream_key 被截图的视频流，参看 RemoteStreamKey{@link #RemoteStreamKey}。
     * @param file_path 截图文件的保存路径。
     * @param width 截图图像的宽度。单位：像素。
     * @param height 截图图像的高度。单位：像素。
     * @param error_code 截图错误码。请参看 SnapshotErrorCode{@link #SnapshotErrorCode}。
     ...
     * @type callback
     * @brief Triggered after calling takeSnapshotToFile{@link #IRTCVideo#takeSnapshotToFile} to report whether the snapshot is taken successfully and provide details of the snapshot image.
     * @param stream_key Information of the snapshotted video stream. See RemoteStreamKey{@link #RemoteStreamKey}.
     * @param file_path The path where the snapshot file is saved.
     * @param width The width (px) of the snapshot image.
     * @param height The height (px) of the snapshot image.
     * @param error_code The error code for the snapshot task. See SnapshotErrorCode{@link #SnapshotErrorCode} for specific indications.
