...
public:
     ...
     * @valid since 3.60.
     * @type api
     * @brief Mutes/unmutes the audio captured when screen sharing.<br>
     *        Calling this method will send muted data instead of the screen audio data, and it does not affect the local audio device capture status and the SDK audio stream publishing status.
     * @param mute Whether to mute the audio capture when screen sharing. <br>
     *        - True: Mute the audio capture when screen sharing. <br>
     *        - False: (Default) Unmute the audio capture when screen sharing.
     * @return
     *        - 0: Success.
     *        - < 0: Failure. See ReturnStatus{@link #ReturnStatus} for more details.
     * @note
     ...
     * @type api
     * @valid Available since 3.60.
     * @brief Adjusts the volume of audio captured during screen sharing.<br>
     *        This method only changes the volume of audio data and does not affect the hardware volume of the local device.
     * @param volume Ratio(%) of capture volume to original volume, in the range [0, 400], with overflow protection.<br>              
     *               To ensure better call quality, we recommend setting the volume to [0, 100].<br>
     *               - 0: Mute
     *               - 100: Original volume
     *               - 400: Four times the original volume with signal-clipping protection
     * @return  
     *        + 0: Success.<br>
     *        + < 0: Fail. See ReturnStatus{@link #ReturnStatus} for more details.
     * @note You can call this API to set the capture volume either before or after enabling screen audio capture.
     */
     ...
     *        - < 0 : Fail. See ReturnStatus{@link #ReturnStatus} for more details
     * @note
     *        - Call startAudioCapture{@link #IRTCEngine#startAudioCapture}to enable the internal audio capture.
     ...
     *         - Mutually exclusive with setVoiceReverbType{@link #IRTCEngine#setVoiceReverbType}, and the effects set later will override the effects set first.
     ...
     *         - Effective for both internal and external audio source.
     *         - Only valid for mono-channel audio.
     *         - Mutually exclusive with setVoiceChangerType{@link #IRTCEngine#setVoiceChangerType}, and the effects set later will override the effects set first.
     * @list Audio Processing
     */
    ...
    */
     ...
     *        - This method can only change the quality of the camera video.
    * @list Video Management
     */
    ...
    *        - < 0 : 调用失败。查看 ReturnStatus{@link #ReturnStatus} 获得更多错误说明。
     ...
     * @deprecated since 3.60, use setLocalVideoSink{@link #IRTCEngine#setLocalVideoSink} instead.
     ...
     * @param source_id ID of the screen sharing object, which can be obtained from the list of shared objects enumerated by getScreenCaptureSourceList{@link #IRTCEngine#getScreenCaptureSourceList}. For more details, see view_t{@link #view_t}.
     ...
     * @param source_id ID of the screen-sharing object. You can get the ID by calling getScreenCaptureSourceList{@link #IRTCEngine#getScreenCaptureSourceList}. Refer to [view_t](70098#view-t) for more details.
     ...
     *        - Before pushing external video frames, you must call setVideoSourceType{@link #IRTCEngine#setVideoSourceType} to turn on external video source capture.
     * @list Custom Stream Processing
     */
     ...
     * @locale en
     * @type api
     * @brief Create a RTC room instance. <br>
     *        This API only returns a RTCroom instance. You still need to call joinRoom{@link #IRTCRoom#joinRoom} to actually create/join the room. <br>
     ...
     *        In multi-room mode, a user can subscribe to media streams in the joined rooms at the same time.
     * @param room_id The string matches the regular expression: `[a-zA-Z0-9_@\-\.]{1,128}`.
     * @param room_config The game room configuration. See GameRoomConfig{@link #GameRoomConfig}.
     ...
     * @locale en
     * @type api
     * @brief Create an RTS room instance. <br>
     *        This API only returns an RTS room instance. You still need to call joinRTSRoom{@link #IRTSRoom#joinRTSRoom} to actually create/join the room. <br>
    ...
    */
     ...
     *        - < 0 : Fail. See ReturnStatus{@link #ReturnStatus} for more details
     * @note
     *       - The default acquisition method is internal to the RTC SDK.
     ...
     *       - Whether it is an internal capture or a custom capture, you must call publishStreamAudio{@link #IRTCRoom#publishStreamAudio} to push the captured screen audio to the remote end.
     */
     ...
     *         - After collection, you also need to call publishStreamAudio{@link #IRTCRoom#publishStreamAudio} to collect the screen audio Push to the far end.
     ...
     *         - After calling this interface to push the custom captured audio frame to the RTC SDK, you must call publishStreamAudio{@link #IRTCRoom#publishStreamAudio} to push the captured screen audio to the remote end. Audio frame information pushed to the RTC SDK is lost before calling publishStreamAudio{@link #IRTCRoom#publishStreamAudio}.
     */
     ...
     *       local users will receive a callback to onMediaDeviceStateChanged. <br>
     ...
     *        - < 0 : Fail. See ReturnStatus{@link #ReturnStatus} for more details
     * @note
     *        - After collection, you also need to call publishScreen to collect the screen audio/vieo Push to the far end.
     ...
     *        - 0: Success.
     *        - < 0 : Fail. See ReturnStatus{@link #ReturnStatus} for more details
     * @note This API must be called after the startScreenCapture function. Local users will receive a callback to onMediaDeviceStateChanged. <br>
     ...
     * @type api
     * @brief Enable audio voice detection. After that, you will receive onAudioVADStateUpdate{@link #IRTCEngineEventHandler#onAudioVADStateUpdate}. 
     * @param interval Callback interval, in milliseconds. <br>
     *       + `<= 0`: Disable callback.
     ...
     * @param interval Callback interval, in milliseconds. <br>
     *       + `<= 0`: Disable callback.
     *       + `[100, 3000]`: Enable callback and set the reporting interval to this value. It is recommended to set it to 2000.
     *       + Invalid interval values: less than 100 set to 100, greater than 3000 set to 3000.  
     * @return
     *        + 0: Success.
     *        + < 0 : Fail. See ReturnStatus{@link #ReturnStatus} for more details.
     ...
     * @hidden(Linux)
     * @type api
     * @brief Takes a snapshot of the remote video stream and save it as a JPG file at the specified local path. <br>
     ...
     * @param stream_id ID of the remote video stream for taking snapshot.
     ...
     * @valid since 3.60.
     * @type api
     * @brief Takes a snapshot of the local video stream and save it as a JPG file at the specified local path. <br>
     *        After calling this method, the SDK triggers onLocalSnapshotTakenToFile{@link #IRTCEngineEventHandler#onLocalSnapshotTakenToFile} to report whether the snapshot is taken successfully and provide details of the snapshot image.
     * @param file_path The absolute file path where the snapshot JPG file will be saved. The file extension must be `.jpg`. Ensure that the directory exists and is writable. Examples: `C:\\Users\\YourName\\Pictures\\snapshot.jpg`.
     * @return The index of the remote snapshot task, starting from `1`. The index can be used to track the task status or perform other management operations.
     * @list Advanced Features
     */
