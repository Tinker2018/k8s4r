...
public:
     ...
     *               测试时可使用[控制台](https://console.volcengine.com/rtc/listRTC?from=doc)生成临时 Token，`roomId` 填任意值。
     ...
     * @note  You will receive onLoginResult{@link #IRTSEventHandler#onLoginResult} after calling this API and log in successfully. But remote users will not receive notification about that.
     */
     ...
     *        This API only returns a room instance. You still need to call joinRoom{@link #IRTCRoom#joinRoom} to actually create/join the room.
     *        Each call of this API creates one IRTSRoom{@link #IRTSRoom} instance. Call this API as many times as the number of rooms you need, and then call joinRoom{@link #IRTCRoom#joinRoom} of each IRTSRoom instance to join multiple rooms at the same time.
     ...
     * @note To set local proxy, call setLocalProxy{@link #IRTS#setLocalProxy}.
     */
     ...
     * @param event_handler Handler sent from SDK to App. See IRTSEventHandler{@link #IRTSEventHandler}.
     * @return  <br>
     ...
     * @param engine The engine instance created by createRTS{@link #createRTS}.
     * @note  <br>
     ...
     * @note This API must be called before createRTS{@link #createRTS}.
     */
